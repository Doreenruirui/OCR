    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__):
            with vs.variable_scope("Attn2"):
                gamma_h = tanh(rnn_cell._linear(state, self._num_units, True, 1.0))
        ...
        gru_out, gru_state = super(GRUCellAttn, self).__call__([inputs, context], state, scope)
            #self.attn_map = tf.squeeze(tf.slice(weights, [0, 0, 0], [-1, -1, 1]))
        return (gru_out, gru_out)
