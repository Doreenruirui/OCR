...
class GRUCellAttn(rnn_cell.GRUCell):
    def __init__(self, num_units, encoder_output, prob_vector, weight=0.5, len_in=60, scope=None):
    ...
    def decode(self, time_step, inputs, state, scope=None):
        gru_out, gru_state = super(GRUCellAttn, self).__call__(inputs, state, scope)
        cur_weights = tf.slice(self.prob, [self.len_in + 13 - time_step], [self.len_in])
        # cur_weights = tf.exp(cur_weights * self.scalar)
        cur_weights = cur_weights / tf.reduce_sum(cur_weights)
        cur_weights = tf.reshape(cur_weights, [-1, 1, 1])
        with vs.variable_scope(scope or type(self).__name__, reuse=True):
            ...
            weights = tf.exp(weights - tf.reduce_max(weights, reduction_indices=0, keep_dims=True))
            weights = weights / (1e-6 + tf.reduce_sum(weights, reduction_indices=0, keep_dims=True))
            weights = weights * self.weight + cur_weights * (1 - self.weight)
            context = tf.reduce_sum(self.hs * weights, reduction_indices=0)
...
class NLCModel(object):
    def __init__(self, vocab_size, size, num_layers, max_gradient_norm, batch_size, learning_rate,
                 learning_rate_decay_factor, dropout, weight=0.5, forward_only=False, optimizer="adam"):
        ...
        self.len_in = tf.placeholder(tf.int32)
        self.weight = weight
    ...
    def decoder_graph(self, decoder_inputs, decoder_state_input, time_step):
        ...
        with vs.variable_scope("Decoder", reuse=True):
            ...
            with vs.variable_scope("DecoderAttnCell") as scope:
                decoder_output, state_output = self.attn_cell.decode(time_step, inp, decoder_state_input[i+1])
                decoder_state_output.append(state_output)
    ...
    def decode_beam(self, session, encoder_output, probs, beam_size, len_input):
        input_feed = {}
        input_feed[self.encoder_output] = encoder_output
        input_feed[self.keep_prob] = 1.
        input_feed[self.beam_size] = beam_size
        input_feed[self.len_input] = len_input
        input_feed[self.probs] = probs
        input_feed[self.len_in] = encoder_output.shape[0]
