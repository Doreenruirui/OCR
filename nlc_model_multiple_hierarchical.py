...
class GRUCellAttn(rnn_cell.GRUCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__):
            ...
            context_w1 = tf.reduce_sum(self.phi_hs * weights, reduction_indices=1)
            weights_ctx = tf.reduce_sum(context_w1 * gamma_h, reduction_indices=2, keep_dims=True)
            weights_ctx = tf.exp(weights_ctx - tf.reduce_max(weights_ctx, reduction_indices=0, keep_dims=True))
            weights_ctx = weights_ctx / (1e-6 + tf.reduce_sum(weights_ctx, reduction_indices=0, keep_dims=True))
            context_w = tf.reduce_sum(context * tf.reshape(weights_ctx, [tf.shape(context)[0], -1, 1]), reduction_indices=0)
            context_w = tf.reshape(context_w, [-1, self._num_units])
            with vs.variable_scope("AttnConcat"):
                out = tf.nn.relu(rnn_cell._linear([context_w, gru_out], self._num_units, True, 1.0))
            return (out, out)
...
class NLCModel(object):
    ...
    def setup_decoder(self):
        ...
        self.attn_cell = GRUCellAttn(self.size, self.encoder_output_feed, scope="DecoderAttnCell")
