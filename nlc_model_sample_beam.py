...
class NLCModel(object):
    ...
    def setup_beam(self):
        self.batch_size = tf.shape(self.encoder_output)[1]
        time_0 = tf.constant(0)
        beam_seqs_0 = tf.ones(shape=(self.batch_size, self.beam_size, 1), dtype=tf.int32) * nlc_data.SOS_ID
        beam_probs_0 = tf.zeros(shape=(self.batch_size, self.beam_size), dtype=tf.float32)
        cand_seqs_0 = tf.ones(shape=(self.beam_size, 1), dtype=tf.int32) * nlc_data.EOS_ID
        cand_probs_0 = tf.ones(shape=(self.batch_size, self.beam_size), dtype=tf.float32) * -3e38
        ...
        def beam_cond(cand_probs, cand_seqs, time, beam_probs, beam_seqs, *states):
            return tf.logical_and(tf.reduce_sum(tf.cast(tf.greater(beam_probs_0, cand_probs_0), tf.int32))
                                  < tf.shape(cand_probs)[0] * tf.shape(cand_probs[1]),
                                  time < tf.reshape(self.len_input, ()) + 10)

        def beam_step(cand_probs, cand_seqs, time, beam_probs, beam_seqs, *states):
            inputs = tf.reshape(tf.slice(beam_seqs, [0, 0, time], [self.batch_size, self.beam_size, 1]), [self.batch_size * self.beam_size])
        ...
        loop_vars = [cand_probs_0, cand_seqs_0, time_0, beam_probs_0, beam_seqs_0] + states_0
