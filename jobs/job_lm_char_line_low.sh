...
start=$(($start + 1))
sed -n ''$start','$end'p' $folder_data'/train.text' |  awk '{ print tolower($0) }' - > $folder_data'/train.text.'$file_no
#awk '{ print tolower($0) }' $folder_data'/train.text.'$file_no > $folder_data'/train.text.'$file_no'.tmp'
