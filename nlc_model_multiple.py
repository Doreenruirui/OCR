...
from model_common import GRUCellAttn, get_optimizer
import nlc_data


class GRUCellAttnML(rnn_cell.GRUCell):
...
class NLCModel(object):
    def __init__(self, vocab_size, size, num_layers, max_gradient_norm, batch_size, learning_rate,
        ...
        self.len_input = tf.placeholder(tf.int32)                  # to constraint the length of beam search

        self.source_tokens = tf.placeholder(tf.int32, shape=[None, None, None])
        self.target_tokens = tf.placeholder(tf.int32, shape=[None, None, None])
        self.source_mask = tf.placeholder(tf.int32, shape=[None, None, None])
        self.target_mask = tf.placeholder(tf.int32, shape=[None, None, None])
        self.target_length = tf.reduce_sum(self.target_mask, reduction_indices=1)
    ...
    def setup_encoder(self):
        encoder_cell = rnn_cell.GRUCell(self.size)
        with vs.variable_scope("PryamidEncoder"):
            max_seq = tf.shape(self.source_tokens)[2]
            inp = tf.reshape(self.encoder_inputs, [-1, self.size])
            mask = tf.reshape(self.source_mask, [-1, max_seq])
            out = None
            for i in xrange(self.num_layers):
                with vs.variable_scope("EncoderCell%d" % i) as scope:
                    srclen = tf.reduce_sum(mask, reduction_indices=1)
                    out, _ = self.bidirectional_rnn(encoder_cell, inp, srclen, scope=scope)
        ...
        with vs.variable_scope("Decoder"):
            ...
            for i in xrange(self.num_layers - 1):
                with vs.variable_scope("DecoderCell%d" % i) as scope:
                    out, state_output = rnn.dynamic_rnn(self.decoder_cell, inp, dtype=dtypes.float32,
            ...
            with vs.variable_scope("DecoderAttnCell") as scope:
                out, state_output = rnn.dynamic_rnn(self.attn_cell, inp, dtype=dtypes.float32,
                                                    sequence_length=self.target_length,
    ...
    def bidirectional_rnn(self, cell, inputs, lengths, scope=None):
        ...
        with vs.variable_scope(name + "_FW") as fw_scope:
            output_fw, output_state_fw = rnn.dynamic_rnn(cell, inputs, dtype=dtypes.float32,
        ...
        with vs.variable_scope(name + "_BW") as bw_scope:
            output_bw, output_state_bw = rnn.dynamic_rnn(cell, inputs_bw, dtype=dtypes.float32,
