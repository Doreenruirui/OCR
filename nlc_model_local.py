...
class GRUCellAttn(rnn_cell.GRUCell):
    def __init__(self, num_units, encoder_output, window_size, scope=None):
        self.hs = encoder_output
        self.batch_size = tf.shape(self.hs)[1]
        self.len_seq = tf.shape(self.hs)[0]
    ...
    def __call__(self, inputs, state, scope=None):
        time_step = tf.cast(tf.reshape(tf.slice(inputs, [0, 0], [1, 1]), []), tf.int32)
        inputs = tf.slice(inputs, [0, 1], [-1, -1])
        gru_out, gru_state = super(GRUCellAttn, self).__call__(inputs, state, scope)
        start = tf.maximum(0, time_step - self.window_size)
        start = tf.minimum(start, self.len_seq - self.window_size * 2 + 1)
        len_seq = tf.minimum(self.len_seq - start, self.window_size * 2 + 1)
        cur_phi_hs = tf.slice(self.phi_hs, [start, 0, 0], [len_seq, -1, -1])
        cur_hs = tf.slice(self.hs, [start, 0, 0], [len_seq, -1, -1])
        with vs.variable_scope(scope or type(self).__name__):
            with vs.variable_scope("Attn2"):
                gamma_h = tanh(rnn_cell._linear(gru_out, self._num_units, True, 1.0))
            weights = tf.reduce_sum(cur_phi_hs * gamma_h, reduction_indices=2, keep_dims=True)
            ...
            context = tf.reshape(tf.reduce_sum(cur_hs * weights, reduction_indices=0), [-1, self._num_units])
...
class NLCModel(object):
    def __init__(self, vocab_size, size, window_size, num_layers, max_gradient_norm, batch_size, learning_rate,
        ...
        self.batch_size = tf.shape(self.source_tokens)[1]
        self.len_seq = tf.shape(self.target_tokens)[0]
    ...
    def setup_decoder(self):
        ...
        self.attn_cell = GRUCellAttn(self.size, self.encoder_output, self.window_size, scope="DecoderAttnCell")
        ...
        with vs.variable_scope("Decoder"):
            ...
            with vs.variable_scope("DecoderAttnCell") as scope:
                index = tf.transpose(tf.reshape(tf.tile(tf.range(self.len_seq),
                                                             [self.batch_size]),
                                                     [-1, self.len_seq]))
                inp = tf.concat(2, [tf.cast(tf.reshape(index,
                                                       [self.len_seq, -1, 1]),
                                            dtype=tf.float32),
                                    inp])
    ...
    def decoder_graph(self, decoder_inputs, decoder_state_input, time):
        decoder_output, decoder_state_output = None, []
        inp = decoder_inputs
        num_inp = tf.shape(decoder_inputs)[0]
        index = tf.ones((num_inp, 1)) * tf.cast(time, tf.float32)
    ...
    def setup_beam(self):
        ...
        def beam_step(cand_probs, cand_seqs, time, beam_probs, beam_seqs, cand_seq_prob, beam_seq_prob, *states):
            ...
            decoder_output, state_output = self.decoder_graph(decoder_input, states, time)
