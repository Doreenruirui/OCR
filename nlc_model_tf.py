...
class NLCModel(object):
    ...
    def setup_encoder(self):
        with vs.variable_scope("PryamidEncoder"):
            ...
            fw_cell = rnn_cell.DropoutWrapper(fw_cell, output_keep_prob=self.keep_prob)
            bw_cell = rnn_cell.DropoutWrapper(bw_cell, output_keep_prob=self.keep_prob)
            fw_cell = rnn_cell.MultiRNNCell([fw_cell] * self.num_layers)
            bw_cell = rnn_cell.MultiRNNCell([bw_cell] * self.num_layers)
            inp = self.encoder_inputs
            srclen = tf.reduce_sum(self.source_mask, reduction_indices=0)
            out, _ = rnn.bidirectional_dynamic_rnn(fw_cell, bw_cell, inp, srclen)
        ...
        with vs.variable_scope("Decoder"):
            ...
            if self.num_layers > 1:
                with vs.variable_scope("DecoderCell"):
                    ...
                    decoder_cell = rnn_cell.DropoutWrapper(decoder_cell, output_keep_prob=self.keep_prob)
                    self.decoder_cell = rnn_cell.MultiRNNCell([decoder_cell] * (self.num_layers - 1))
                    inp, _ = rnn.dynamic_rnn(self.decoder_cell, inp, time_major=True,
                                                dtype=dtypes.float32, sequence_length=self.target_length)
            with vs.variable_scope("DecoderAttnCell") as scope:
                self.attn_cell = GRUCellAttn(num_units=self.size, encoder_output=self.encoder_output)
                out, _ = rnn.dynamic_rnn(self.attn_cell, inp, time_major=True,
                                                    dtype=dtypes.float32, sequence_length=self.target_length,
    ...
    def decoder_graph(self, decoder_inputs, decoder_state_input):
        ...
        with vs.variable_scope("Decoder", reuse=True):
            with vs.variable_scope("DecoderCell"):
                inp, _ = self.decoder_cell(inp)
                decoder_state_output.append(inp)

            with vs.variable_scope("DecoderAttnCell"):
                decoder_output, _ = self.attn_cell(inp)
                decoder_state_output.append(decoder_output)
