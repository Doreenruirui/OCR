...
class NLCModel(object):
    ...
    def setup_beam(self):
        ...
        probs_0 = tf.constant([[1]], dtype=tf.int32)
        total_probs_0 = tf.zeros((1, self.beam_size))
        ...
        def beam_cond(total, probs, time, beam_probs, beam_seqs, cand_probs, cand_seqs, *states):
            return tf.reduce_max(beam_probs) >= tf.reduce_min(cand_probs)

        def beam_step(total, probs, time, beam_probs, beam_seqs, cand_probs, cand_seqs, *states):
            ...
            return [next_total, probs, time + 1, next_beam_probs, next_beam_seqs, next_cand_probs, next_cand_seqs] + next_states


        var_shape = []
        var_shape.append((total_probs_0, tf.TensorShape([None, None])))
        var_shape.append((probs_0, tf.TensorShape([None, None])))
        ...
        cand_seqs = ret_vars[6]
        cand_probs = ret_vars[5]
