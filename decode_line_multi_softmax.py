...
def decode():
    ...
    if flag_evl:
        ...
        f_o = open(pjoin(folder_out, FLAGS.dev + '.soft' + '.ec.txt.' + str(FLAGS.start) + '_' + str(FLAGS.end)), 'w')
        pool = Pool(100)
    else:
        f_o = open(pjoin(folder_out, FLAGS.dev + '.soft' + '.o.txt.' + str(FLAGS.start) + '_' + str(FLAGS.end)), 'w')
