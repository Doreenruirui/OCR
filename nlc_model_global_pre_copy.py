...
class NLCModel(object):
    ...
    def setup_loss(self):
        with vs.variable_scope("Logistic"):
            ...
            #outputs2d = tf.reshape(tf.nn.log(tf.clip_by_value(vocab_prob, 1e-10, 1.0)), [-1, self.vocab_size])
    ...
    def train(self, session, source_tokens, source_mask, target_tokens, target_mask):
        ...
        output_feed = [self.updates, self.gradient_norm, self.losses, self.param_norm, self.pre2d]
