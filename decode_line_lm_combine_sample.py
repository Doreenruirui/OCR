...
def lm_rank_all(strs, probs):
    ...
    #    print(s, p, l)
    rescores = [(1 - a - b) * q + a * l + b * p for (l, p, q) in zip(lmscore1, lmscore2, probs)]
    rerank = [rs[0] for rs in sorted(enumerate(rescores), key=lambda x: x[1])]
    generated = strs[rerank[-1]]
    # lm_score1 = lmscore1[rerank[-1]]
    # lm_score2 = lmscore2[rerank[-1]]
...
def fix_sent(model, sess, sent):
    ...
    best_str = lm_rank_all(beam_strs, probs)
    # Return
    #return best_str
...
def decode():
    ...
    with open(pjoin(FLAGS.data_dir, 'dev.y.txt'), 'r') as f_:
        truths = f_.readlines()
    folder_out = pjoin(FLAGS.data_dir, str(FLAGS.beam_size) + '_' + str(FLAGS.alpha) + '_' + str(FLAGS.beta))
