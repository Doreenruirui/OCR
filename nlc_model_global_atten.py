...
class GRUCellAttn(rnn_cell.GRUCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__):
            ...
            context = tf.reshape(tf.reduce_sum(self.hs * weights, reduction_indices=0), [-1, self._num_units])
            with vs.variable_scope("AttnConcat"):
                out = tf.nn.relu(rnn_cell._linear([context,
                                                   gru_out,
                                                   tf.reshape(tf.tile(self.attn, [batch_size, 1]), [-1, self._num_units])], self._num_units, True, 1.0))
