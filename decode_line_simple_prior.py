...
def fix_sent(model, sess, sent):
    ...
    probs_v = gs(len_input * 2 + 13 * 2 + 1, FLAGS.variance)
...
def decode():
    ...
    f_o = open(pjoin(folder_out, FLAGS.dev + '.prior.o.txt.' + str(FLAGS.start) + '_' + str(FLAGS.end)), 'w')
    f_p = open(pjoin(folder_out, FLAGS.dev + '.prior.p.txt.' + str(FLAGS.start) + '_' + str(FLAGS.end)), 'w')
