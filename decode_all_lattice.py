...
def decode_beam(model, sess, encoder_output, max_beam_size):
    toks, probs, trans = model.decode_beam(sess, encoder_output, beam_size=max_beam_size)
    return toks.tolist(), probs.tolist(), trans
...
def decode_lattice(beamstrs, probs, prob_trans):
    ...
    for i in range(num_str):
        cur_str = beamstrs[i]
        if abs(sum(prob_trans[i]) - probs[i]) < 1e-6:
            ...
            prefix = cur_str + '<eos>'
            if prefix not in dict_str:
                dict_str[prefix] = prob_trans[i][len(cur_str) + 1]
    return dict_str
...
def fix_sent(model, sess, sent):
    ...
    # Decode
    beam_toks, probs, prob_trans = decode_beam(model, sess, encoder_output, FLAGS.beam_size)
    # De-tokenize
...
def decode():
    ...
    x_train, y_train, x_dev, y_dev, vocab_path = nlc_data.prepare_nlc_data(
        pjoin(FLAGS.data_dir, FLAGS.tokenizer.lower()), FLAGS.max_vocab_size,
    ...
    with tf.Session() as sess:
        ...
        with open(pjoin(FLAGS.train_dir, FLAGS.tokenizer.lower(), 'test.o.txt'), 'w') as f_:
            for line in file(pjoin(FLAGS.data_dir, FLAGS.tokenizer.lower(), 'test.x.txt')):
                # sent = raw_input("Enter a sentence: ")
                sent = line.strip()
                output_sent = fix_sent(model, sess, sent)
                f_.write(output_sent + '\n')
