...
class NLCModel(object):
    ...
    def setup_beam(self):
        ...
        batch_size = tf.shape(self.encoder_output)[0]
        time_0 = tf.constant(0)
        beam_seqs_0 = tf.ones(shape=(1, batch_size, beam_size), dtype=tf.int32) * nlc_data.SOS_ID
        beam_probs_0 = tf.zeros(shape=(1, batch_size, beam_size), dtype=tf.float32)
        cand_seqs_prob_0 = tf.constant(shape=(1, batch_size, beam_size))
